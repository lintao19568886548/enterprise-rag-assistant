...
class NodeRrf(NodeBase):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        功能：
        将来自不同检索源（如 Embedding 检索、HyDE 检索等）的结果进行融合排序。
        ...
        embedding_chunks = self._as_entity_list(state.get("embedding_chunks"))
        hyde_embedding_chunks = self._as_entity_list(state.get("hyde_embedding_chunks"))
        ...
        rrf_res = self._reciprocal_rank_fusion(source_weights, k=60, max_results=10)
    ...
    def _as_entity_list(self, state_list) -> List[Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    ...
    try:
        # 运行节点
        node_rrf = NodeRrf()
        final_state = node_rrf(init_state)
