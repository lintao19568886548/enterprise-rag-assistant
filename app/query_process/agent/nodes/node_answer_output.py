...
from app.utils.task_utils import set_task_result, add_done_task
    ...
    def _step_3_generate_response(self, state: QueryGraphState, prompt: str) -> QueryGraphState:
        ...
        if is_stream:
            ...
            try:
                ...
                logger.opt(exception=True).error("流式生成出错: {}", e)
                # 发生错误时，尝试推送到前端
            ...
            try:
                ...
                logger.opt(exception=True).error("生成回答出错: {}", e)
...
# if __name__ == "__main__":
#
#     logger.info("开始测试: 答案生成节点 (AnswerOutputNode)")
#
#     # 构造模拟状态
#     mock_state = {
...
#
#     logger.info("【输入状态】:")
#     logger.info(f"  query: {mock_state['rewritten_query']}")
#     logger.info(f"  item_names: {mock_state['item_names']}")
#     logger.info(f"  reranked_docs: {len(mock_state['reranked_docs'])} 篇")
#
#     # 执行答案生成
#     node_answer_output = NodeAnswerOutput()
#     result = node_answer_output(mock_state)
...
#     # 打印结果
#     logger.info("【生成结果】:")
#     logger.info(result.get("answer", "无答案"))
#     logger.info("测试完成")
